...
def _mean_absolute_error(y, y_pred, w):
    ...
    return np.average(np.abs(y_pred - y), weights=w, axis=0).mean()


def _mean_square_error(y, y_pred, w):
    """Calculate the mean square error."""
    return np.average(((y_pred - y) ** 2), weights=w, axis=0).mean()


def _root_mean_square_error(y, y_pred, w):
    """Calculate the root mean square error."""
    return np.sqrt(np.average(((y_pred - y) ** 2), weights=w,axis=0).mean())
...
def _scipy_ic(y, y_pred, w):
    coef = []
    for i in range(y.shape[0]):
        coef.append(pearsonr(y[i,:], y_pred[i,:], nan_policy='omit')[0])

    weight = pd.DataFrame(w).sum(axis=1).astype(bool)
    coef = pd.Series(coef).fillna(0).loc[weight]
    return coef.mean()

def _scipy_rank_ic(y, y_pred, w):
    coef = []
    for i in range(y.shape[0]):
        coef.append(spearmanr(y[i,:], y_pred[i,:], nan_policy='omit')[0])

    weight = pd.DataFrame(w).sum(axis=1).astype(bool)
    coef = pd.Series(coef).fillna(0).loc[weight]
    return coef.mean()

def _scipy_rank_ir(y, y_pred, w):
    coef = []
    for i in range(y.shape[0]):
        coef.append(spearmanr(y[i,:], y_pred[i,:], nan_policy='omit')[0])
...
log_loss = _Fitness(function=_log_loss,
                    greater_is_better=False)
scipy_ic = _Fitness(function=_scipy_ic,
                            greater_is_better=True)
scipy_rank_ic = _Fitness(function=_scipy_rank_ic,
                            greater_is_better=True)
scipy_rank_ir = _Fitness(function=_scipy_rank_ir,
                            greater_is_better=True)
...
_fitness_map = {'pearson': weighted_pearson,
                ...
                'log loss': log_loss,
                'scipy-ic':scipy_ic,
                'scipy-rank_ic':scipy_rank_ic,
                'scipy-rank_ir':scipy_rank_ir,
                }
