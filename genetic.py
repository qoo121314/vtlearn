...
from .functions import _extended_function_map, _Function, sig1 as sigmoid
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        if oob_method == 'tail':
            tmp = np.repeat(False, n_samples)
            tmp[max_samples:] = True
            not_indices = tmp
            indices = ~tmp
        else:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for function in self.function_set:
            if isinstance(function, str):
                if function not in _extended_function_map:
                    raise ValueError('invalid function name %s found in '
                                     '`function_set`.' % function)
                self._function_set.append(_extended_function_map[function])
        ...
        if isinstance(self.metric, _Fitness):
            ...
            if self.metric not in ('mean absolute error', 'mse', 'rmse',
                                   'pearson', 'spearman', 'scipy-ic', 'scipy-rank_ic', 'scipy-rank_ir'):
            ...
            if self.metric not in ('pearson', 'spearman', 'scipy-ic', 'scipy-rank_ic', 'scipy-rank_ir'):
                raise ValueError('Unsupported metric: %s' % self.metric)
            self._metric = _fitness_map[self.metric]
